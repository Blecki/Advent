    ...
    internal class Problem06
    {
        public class AncoraGrammar : Ancora.Grammar
        {
            public AncoraGrammar()
            {
                ...
                var number = Token(c => "0123456789".Contains(c));
                var coordinate = number + Character(',').WithMutator(Discard()) + number;
                var coordinatePair = (coordinate + ws + Keyword("through").WithMutator(Discard()) + ws + coordinate).WithMutator(n =>
                {
                    n.Children.AddRange(n.Children[2].Children);
                    n.Children.RemoveAt(2);
                    return n;
                });
                var toggle = (ws + Keyword("toggle") + ws + coordinatePair + ws).WithMutator(n => { n.Children[1].NodeType = "toggle"; return n.Children[1]; });
                var turnOff = (ws + Keyword("turn off") + ws + coordinatePair + ws).WithMutator(n => { n.Children[1].NodeType = "off"; return n.Children[1]; });
                var turnOn = (ws + Keyword("turn on") + ws + coordinatePair + ws).WithMutator(n => { n.Children[1].NodeType = "on"; return n.Children[1]; });
                Root = (toggle | turnOff | turnOn).WithMutator(Collapse());
            }
        }
        ...
        public static void Solve()
        {
            ...
            foreach (var l in lines)
            {
                ...
                var x1 = Int32.Parse(ast.Node.Children[0].Value.ToString());
                var y1 = Int32.Parse(ast.Node.Children[1].Value.ToString());
                var x2 = Int32.Parse(ast.Node.Children[2].Value.ToString());
                var y2 = Int32.Parse(ast.Node.Children[3].Value.ToString());

                if (ast.Node.NodeType == "toggle")
                    for (var x = x1; x <= x2; ++x) for (var y = y1; y <= y2; ++y) lights[x,y] = !lights[x,y];
                else if (ast.Node.NodeType == "off")
                    for (var x = x1; x <= x2; ++x) for (var y = y1; y <= y2; ++y) lights[x,y] = false;
                else if (ast.Node.NodeType == "on")
                    for (var x = x1; x <= x2; ++x) for (var y = y1; y <= y2; ++y) lights[x,y] = true;
            }

            var lit = 0;
            for (var x = 0; x < 1000; ++x) for (var y = 0; y < 1000; ++y)
                    if (lights[x, y]) lit += 1;

            Console.WriteLine("Lights lit: {0}", lit);
        ...
        public static void Solve2()
        {
            ...
            foreach (var l in lines)
            {
                ...
                if (ast.Node.NodeType == "toggle")
                    for (var x = x1; x <= x2; ++x) for (var y = y1; y <= y2; ++y) lights[x, y] += 2;
                else if (ast.Node.NodeType == "off")
                    for (var x = x1; x <= x2; ++x) for (var y = y1; y <= y2; ++y) { if (lights[x, y] > 0) lights[x, y] -= 1; }
                else if (ast.Node.NodeType == "on")
                    for (var x = x1; x <= x2; ++x) for (var y = y1; y <= y2; ++y) lights[x, y] += 1;
            }
            ...
            Console.WriteLine("Total brightness: {0}", lit);
        }
